...
public class Message {
    ...
    public void saveMessage(String db_url, String db_login, String db_pass) throws SQLException {
        ...
        PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO messages (msg, to_user, from_user, type) VALUES (?,?,?,?)");
    ...
    public static ArrayList<Message> readPublicMessages(String db_url, String db_login, String db_pass) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            Message message;
            if(type == 1)
            message = new Message(name + ": " + msg, fromUser, toUser, type);
            else
            message = new Message(msg, fromUser, toUser, type);
            messages.add(message);
        }
        statement.close();
        return messages;
    }
    public static ArrayList<Message> readPersonalMessages(String db_url, String db_login, String db_pass, int fromUser, int toUser) throws SQLException {
        Connection connection = DriverManager.getConnection(db_url, db_login, db_pass);
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery(
                "SELECT * FROM `messages`, `users` WHERE (from_user='"+fromUser+"' AND to_user'"+toUser+"' " +
                        "OR from_user='"+toUser+"' AND to_user'"+fromUser+"') AND users.id = messages.from_user");
        ArrayList<Message> messages = new ArrayList<>();
        while (resultSet.next()) {
            String msg = resultSet.getString("msg");
            String name = resultSet.getString("name");
